...
def run(
    ...
    for ix, img in enumerate(img_list):
        print('Image ' + img + ' [' + str(ix + 1) + '/' + str(len(img_list)) + ']')
        ...
        if img_name not in pred_dict and img_name in truth_dict:
            results[img_name] = {'FN': len(truth_dict[img_name])}  # Associate number of FNs per image
        ...
        if img_name not in truth_dict and img_name in pred_dict:
            results[img_name] = {'FP': len(pred_dict[img_name])}  # Associate number of FPs per image
        ...
        if img_name in truth_dict and img_name in pred_dict:
            ...
            # It's possible that the no. of predict labels is less
            # than the number of truth labels. In this case the difference counts as people not detected, or FN
            num_fn = len(truth_labels) - len(pred_labels)

            results[img_name] = {'FP': (num_fp if num_fp > 0 else 0), 'TP': (num_tp if num_tp > 0 else 0),
                                 'FN': (num_fn if num_fn > 0 else 0)}
